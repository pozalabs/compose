...
compose.dishka.create_auto_wired_route(...)  # 사용 계층: compose 제공
```
...
router = APIRouter(route_class=compose.dishka.create_auto_wired_route(container))
...
router = APIRouter(
    route_class=compose.dishka.create_auto_wired_route(container),
    prefix="/playlists",
)
