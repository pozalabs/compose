...
- `compose.dishka`: 신규 최상위 모듈
- `compose.dishka.fastapi`: FastAPI 통합 (route class 등)
- `compose.dependency`, `compose.fastapi`의 기존 DI 코드: 변경 없음
- 두 모듈은 독립적으로 존재하며 서로 의존하지 않음
...
compose.dishka.fastapi.create_auto_wired_route(...)  # 사용 계층: compose 제공
```

### 공개 API

#### `create_auto_wired_route(container)`

dishka의 `DishkaRoute`를 확장하여, 엔드포인트 파라미터 중 container가 해결 가능한 타입을 자동으로 `FromDishka[Type]`으로 변환하는 route class를 반환하는 팩토리.

- container에 등록된 타입이면 변환 대상 (Generic, Protocol 포함). 단, 어노테이션 없음, Any, Optional, Union, Annotated는 명시적으로 제외
...
router = APIRouter(route_class=compose.dishka.fastapi.create_auto_wired_route(container))
...
소비자 프로젝트에서 dishka 컨테이너를 생성하고 FastAPI에 연결하는 코드는 compose가 래핑하지 않는다. `setup_dishka`와 `create_auto_wired_route`를 함께 사용한다.
...
router = APIRouter(
    route_class=compose.dishka.fastapi.create_auto_wired_route(container),
    prefix="/playlists",
)
...
- 항상: container에 등록된 타입이면 변환 대상. 어노테이션 없음, Any, Optional, Union, Annotated는 명시적 제외
- 먼저: 기존 프로젝트 전환 전에 신규 프로젝트에서 충분히 검증한다
- 절대: `compose.dependency`와 `compose.dishka`가 서로 의존하지 않는다
...
- `create_auto_wired_route`로 생성한 route class가 DishkaRoute를 확장하는지 확인
